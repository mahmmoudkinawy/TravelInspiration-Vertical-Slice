...
public sealed class ModelValidationBehaviour<TRequest, IResult>(IEnumerable<IValidator<TRequest>> validators) : IPipelineBehavior<TRequest, IResult>
	where TRequest : IRequest<IResult>
{
	...
	public async Task<IResult> Handle(TRequest request, RequestHandlerDelegate<IResult> next, CancellationToken cancellationToken)
	{
		...
		var groupedValidationFailures = validationResults
			...
			.Select(g => new { PropertyName = g.Key, ValidationFailures = g.Select(v => new { v.ErrorMessage }) })
			.ToList();
