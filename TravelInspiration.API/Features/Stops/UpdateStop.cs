...
public sealed class UpdateStop : ISlice
{
	...
	public sealed class StopDto
	{
		...
		public required bool? Suggested { get; set; }
	}
	...
	public sealed class UpdateStopCommandHandler(TravelInspirationDbContext dbContext, IMapper mapper) : IRequestHandler<UpdateStopCommand, IResult>
	{
		...
		{
			var stopToUpdate = await _dbContext.Stops.FirstOrDefaultAsync(
				s => s.Id == request.StopId && s.ItineraryId == request.ItineraryId,
	...
	public sealed class SuggestStopStopUpdatedEventHandler(ILogger<SuggestStopStopUpdatedEventHandler> logger)
